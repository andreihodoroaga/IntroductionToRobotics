...
void Menu::handleJoystickBrightnessUpdates(int valueMultiplier) {
  if (currMenuOption == 2 && currMenuSubOption == 0) {
    if (currMenuBombRow == 0) {
      eepromHandler.updateLCDBrightness(lcdUpdateStep * valueMultiplier, minLcdBrightness, maxLcdBrightness);
    }
    if (currMenuBombRow == 1) {
      eepromHandler.updateMatrixBrightness(matrixUpdateStep * valueMultiplier, minMatrixBrightness, maxMatrixBrightness);
    }
  }
}
